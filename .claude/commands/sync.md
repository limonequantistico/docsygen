...
6. **Changelog update** — Check the commits reviewed against `.docs/changelog.md`. If there's meaningful work done (features built, bugs fixed, refactors) that isn't in the changelog, propose adding it. This keeps a rough log of what was done even when the user didn't go through the backlog.
