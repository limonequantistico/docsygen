...
- **The right gallery for this product type**, not a generic list — [Mobbin](https://mobbin.com/) for real mobile and web app flows, [Dribbble](https://dribbble.com/) for visual direction (aspirational, often impractical), [21st.dev](https://21st.dev/community/components) for component-level patterns, [Land-book](https://land-book.com/) for marketing and landing pages, [Pinterest](https://www.pinterest.com/) for loose moodboarding across all of these (broad style searches surface work the curated galleries miss), and the platform's own guidance (Apple HIG, Material) when convention matters more than distinctiveness.
...
- **Search keywords to paste**, 5-8 of them, built from this project rather than generic terms. Combine the product type, the screen, and a mood or style: "habit tracker app onboarding", "B2B analytics dashboard dark", "recipe app minimal UI". Match the keyword to the gallery — Mobbin searches by app, screen, and flow ("paywall", "empty state", "settings"), while Dribbble and Pinterest respond to style terms ("bento grid", "editorial layout", "soft brutalism").
...
- **Don't recommend the default by accident.** Every model reaches for the same handful of choices when nothing pushes back: Inter for everything, a purple-to-blue accent, soft shadows on rounded cards, dark mode because it "looks technical". Any of these can be the right call — but only when you can say why it fits *these* users. If your recommendation is the one you'd give for any product, it isn't a recommendation yet.
...
2. **Color strategy** — before specific hex values: is this monochrome with one accent, a two-color brand pairing, or a fuller palette? Light-first, dark-first, or both from the start — and does anyone need high contrast or a second brand theme? Ask about existing brand colors before inventing any. A second mode isn't the first one inverted: if the answer is more than one mode, design each mode's values here, not later.
...
Once the decisions are made, open with a one-line **design read** — *"Reading this as: \<product type> for \<audience>, with a \<direction> language."* — adding *"that avoids \<default>"* when the direction is steering away from one. If that line sounds wrong to the user, the tokens will be wrong too, and it's much cheaper to catch here.

Then present a compact summary: the palette with hex values, the type pairing and scale, spacing base, radii, and elevation. Concrete values, so the user is approving a real thing rather than adjectives.
...
4. **Accessibility Checklist**
   ...
   - Verify the chosen palette actually clears contrast targets, in every mode. If a brand color fails against its intended background, say so now and propose a corrected pair — this is the cheapest moment to catch it.
