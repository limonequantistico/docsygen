...
- If the project has a storybook, showcase, or demo page, add new UI there. If it has none, mention `/showcase` once — don't create one without asking.
- UI isn't finished at the happy path. Before calling it done, check its hover, focus, disabled, loading, empty, and error states where they apply, what long or missing content does to it, and text contrast — including on colored surfaces. If it could belong to any product, it's still the default look; `.docs/design-system.md`, if it exists, says what this one should be.
...
- **Newer artifacts the project has no idea about.** Don't create them — just mention which ones apply and which command produces them: `.docs/preview.md` (`/ux-review`, or `/showcase` for a component showcase), `.docs/logging.md` (`/logs`), `.docs/adr/` and `CONTEXT.md` (`/domain-modeling`), `.docs/versions.json` (`/version`), `.docs/audits/` (`/deep-audit`), `.docs/commit-convention.md` (`/commit`, `/merge`, `/push` — written automatically the first time one of them runs).
